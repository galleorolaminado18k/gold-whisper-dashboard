'use client';
// lib/crm/campaignAttribution.ts
// Utilidades para detectar campaña y categoría desde mensajes (UTM/keywords)

import { fetchAllAccountsCampaigns, MetaCampaign } from "@/lib/meta/metaAds";
...
export function getCategoryLabelForMessages(msgs: SimpleMsg[]): string | null {
  ...
  const t = normalize(text);
  if (KW_BALINERIA.some((k) => t.includes(normalize(k)))) return "category:balineria";
  if (KW_JOYERIA.some((k) => t.includes(normalize(k)))) return "category:joyeria";
